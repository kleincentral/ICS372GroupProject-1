...
public class ConsoleUI {
    ...
    private final String saveFile = "orders.txt";

    public void run() {
        List<Order> loadedOrders = persistence.loadOrders(saveFile);
        ...
        persistence.saveOrders(repository.getAllOrders(), saveFile);
    ...
    private void cancelOrder() {
        ...
        System.out.print("Enter cancel reason: ");
    ...
    private void exportOrders() {
        ...
        try (PrintWriter writer = new PrintWriter(new FileWriter(path))) {
            ...
            for (int i = 0; i < orders.size(); i++) {
                ...
                writer.println("      \"type\": \"" + order.getType() + "\",");
                writer.println("      \"status\": \"" + order.getStatus() + "\",");
                writer.println("      \"time\": " + order.getOrderTimeMillis() + ",");
                writer.println("      \"source\": \"" + order.getSource() + "\",");
                writer.println("      \"warehouse\": \"" + order.getWarehouse() + "\",");
                writer.println("      \"items\": [");

                List<org.example.model.Item> items = order.getItems();

                for (int j = 0; j < items.size(); j++) {
                    org.example.model.Item item = items.get(j);
